...
public class TicketServiceImpl implements TicketService {
    ...
    public TicketResponse createTicket(TicketCreateRequest request, User currentUser, String ipAddress) {
        ...
        try {
            auditService.log("Ticket", ticket.getId().toString(), "CREATE", currentUser, "Création " + ticketNumber,
                    ipAddress);
        } catch (Exception e) {
    ...
    private Specification<Ticket> buildSearchSpecification(
        ...
        return (root, query, cb) -> {
            ...
            if (slaStatus != null) {
                List<TicketStatus> terminal = List.of(TicketStatus.CLOSED, TicketStatus.RESOLVED,
    ...
    public TicketResponse changeStatus(Long ticketId, TicketStatusChangeRequest request,
        ...
        if (newStatus == TicketStatus.RESOLVED) {
            ...
            if (request.getFinalCategory() != null)
                ticket.setFinalCategory(request.getFinalCategory());
            if (request.getTimeSpentMinutes() != null)
                ticket.setTimeSpentMinutes(request.getTimeSpentMinutes());
    ...
    public void hardDeleteTicketAsAdmin(
        ...
        cleanupTicketDependencies(ticket);
        ...
        try {
            ...
            log.warn(
                    "Hard delete blocked by data integrity for ticket {}: {}",
                    ticketNumber,
                    exception.getMostSpecificCause() != null
                            ? exception.getMostSpecificCause().getMessage()
                            : exception.getMessage());
            throw new BadRequestException(
                    "Suppression definitive impossible: des references techniques residuelles existent encore pour ce ticket.");
        }
    }
     ...
     * - Les CLIENTs ne peuvent PAS créer de notes internes (isInternal = false
     * forcé)
    ...
    public byte[] exportTicketsCsv(User currentUser, String searchTerm, TicketStatus status,
        ...
        csv.append(
                "Numéro;Titre;Statut;Priorité;Catégorie;Client;Service;Assigné à;Créé le;Deadline SLA;SLA dépassé\r\n");
        for (Ticket t : tickets) {
            ...
            csv.append(escapeCsv(
                    t.getClient() != null && t.getClient().getUser() != null ? t.getClient().getUser().getFullName()
                            : ""))
                    .append(';');
    ...
    public byte[] exportTicketsExcel(User currentUser, String searchTerm, TicketStatus status,
        ...
        try (Workbook wb = new XSSFWorkbook()) {
            ...
            String[] headers = { "Numéro", "Titre", "Statut", "Priorité", "Catégorie", "Client", "Service", "Assigné à",
                    "Créé le", "Deadline SLA", "SLA dépassé" };
            ...
            for (Ticket t : tickets) {
                ...
                row.createCell(5)
                        .setCellValue(t.getClient() != null && t.getClient().getUser() != null
                                ? t.getClient().getUser().getFullName()
                                : "");
    ...
    public byte[] exportTicketsPdf(User currentUser, String searchTerm, TicketStatus status,
        ...
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            document.add(new Paragraph("Export tickets - "
            ...
            for (String h : new String[] { "Numéro", "Titre", "Statut", "Priorité", "Catégorie", "Client", "Service",
                    "Assigné", "Créé", "Deadline", "SLA" }) {
    ...
    public TicketResponse applyMacro(Long ticketId, ApplyMacroRequest request, User currentUser, String ipAddress) {
        ...
        if ("solution".equals(target)) {
            ...
            ticketRepository.save(ticket);
            createHistoryEntry(ticket, currentUser, TicketAction.UPDATE, null, "Macro appliquée (solution)",
                    "Macro: " + macro.getName(), ipAddress);
        } else {
    ...
    public TicketResponse addAttachment(Long ticketId, MultipartFile file, User currentUser) {
        ...
        try {
            ...
            log.info("Attachment {} added to ticket {} by {}", originalFilename, ticket.getTicketNumber(),
    ...
    /**
     * Résout les heures SLA: config (priorité+service) > config (priorité) >
     * props/enum.
     */
    ...
    private void cleanupTicketDependencies(Ticket ticket) {
        detachTicketFromIncidents(ticket.getId());
        slaTimelineRepository.deleteByTicketId(ticket.getId());
        historyRepository.deleteByTicketId(ticket.getId());
        cleanupTicketNotifications(ticket.getId());
        cleanupTicketUploadDirectory(ticket.getId());
    }

    private void detachTicketFromIncidents(Long ticketId) {
        Map<Long, com.billcom.mts.entity.Incident> impactedIncidents = new LinkedHashMap<>();
        ...
        for (com.billcom.mts.entity.Incident incident : impactedIncidents.values()) {
            ...
            if (incident.getTicket() != null && Objects.equals(incident.getTicket().getId(), ticketId)) {
                incident.setTicket(null);
                changed = true;
            }
            if (incident.getTickets() != null) {
                changed = incident.getTickets().removeIf(linkedTicket -> Objects.equals(linkedTicket.getId(), ticketId))
                        || changed;
            }
            if (changed) {
     ...
     * 
     * LOGIQUE:
     * 1. Récupère tous les tickets dont la deadline est dépassée et non marqués
     * breach
     ...
     * 
     * CRITICAL FIX: Added error handling to prevent task from being disabled on
     * exceptions
     */
        ...
        try {
            ...
            for (Ticket ticket : overdueTickets) {
                if (ticket.getBreachedSla() == null || !ticket.getBreachedSla()) {
                    ...
                    ticketRepository.save(ticket);
                    // Use the ticket creator as the history user (system action attributed to
                    // creator)
                    ...
                    if (historyUser != null) {
                        ...
                        log.warn("Cannot create SLA breach history for ticket {} - no user found",
                                ticket.getTicketNumber());
    ...
    /**
     * Job: Détecte les tickets "à risque" (>= 80% du SLA consommé) et envoie une
     * notification.
     * On ne notifie qu'une fois par ticket (slaWarningNotifiedAt) pour éviter le
     * spam.
     * 
     * CRITICAL FIX: Added error handling to prevent task from being disabled on
     * exceptions
    ...
    public void detectSlaAtRisk() {
        try {
            ...
            for (Ticket ticket : active) {
                ...
                if (ticket.getBreachedSla() != null && ticket.getBreachedSla())
                    continue;
                if (ticket.getDeadline() == null || now.isAfter(ticket.getDeadline()))
                    continue;
