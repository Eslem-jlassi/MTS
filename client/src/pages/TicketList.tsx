...
import { Button, Card, Badge, EmptyState, SkeletonTable, DataTable, DataTableColumn } from "../components/ui";
...
const statusVariant: Record<TicketStatus, "default" | "success" | "warning" | "danger" | "info" | "neutral"> = {
...
const SlaIndicator: React.FC<{ 
  ...
  const formatRemaining = (min?: number): string => {
    ...
    const m = absMin % 60;
    return min < 0 ? `-${h}h${m.toString().padStart(2, "0")}` : `${h}h${m.toString().padStart(2, "0")}`;
  };
  ...
  if (percentage !== undefined) {
    const color = slaWarning || percentage > 75 ? "text-warning-600 dark:text-warning-400" : percentage > 50 ? "text-accent-600 dark:text-accent-400" : "text-success-600 dark:text-success-400";
    const barColor = slaWarning || percentage > 75 ? "bg-warning-500" : percentage > 50 ? "bg-accent-500" : "bg-success-500";
    return (
      <div className="flex flex-col items-start">
        ...
        <div className="w-full bg-ds-elevated rounded-full h-1 mt-1" style={{ minWidth: "48px" }}>
          <div className={`h-1 rounded-full ${barColor}`} style={{ width: `${Math.min(100, percentage)}%` }} />
        </div>
      </div>
    );
  }
...
const TicketList: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (searchParams.get("priority")) urlFilters.priority = searchParams.get("priority") as TicketPriority;
    if (searchParams.get("category")) urlFilters.category = searchParams.get("category") as TicketCategory;
    if (searchParams.get("search")) urlFilters.search = searchParams.get("search") || undefined;
    if (searchParams.get("slaStatus")) urlFilters.slaStatus = searchParams.get("slaStatus") as SlaStatusFilter;
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <form onSubmit={handleSearch} className="flex-1">
            <div className="relative">
              <Search size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-ds-muted" />
              <input
                type="text"
                value={searchTerm}
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-ds-elevated">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-ds-muted uppercase tracking-wider">Numéro</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-ds-muted uppercase tracking-wider">Titre</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-ds-muted uppercase tracking-wider">Client</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-ds-muted uppercase tracking-wider">Service</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-ds-muted uppercase tracking-wider">Priorité</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-ds-muted uppercase tracking-wider">Statut</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-ds-muted uppercase tracking-wider">SLA</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-ds-muted uppercase tracking-wider">Assigné à</th>
                    <th className="px-6 py-3 text-right text-xs font-medium text-ds-muted uppercase tracking-wider">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-ds-border">
                  {tickets.map((ticket) => (
                    <tr key={ticket.id} className="hover:bg-ds-elevated/50 transition-colors cursor-pointer" onClick={() => openDrawer(ticket.id)}>
                      <td className="px-6 py-4">
                        <button onClick={(e) => { e.stopPropagation(); openDrawer(ticket.id); }} className="text-primary hover:text-primary-hover font-medium">
                          {ticket.ticketNumber}
                        </button>
                      </td>
                      <td className="px-6 py-4 text-sm text-ds-primary max-w-xs truncate">{ticket.title}</td>
                      <td className="px-6 py-4 text-sm text-ds-secondary">{ticket.clientCompanyName || ticket.clientCode}</td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <button onClick={(e) => { e.stopPropagation(); openDrawer(ticket.id); }} className="text-primary hover:text-primary-hover p-2 inline-block rounded-xl hover:bg-ds-elevated transition-colors">
                          <Eye size={18} />
                        </button>
                      </td>
                    </tr>
            ...
            <div className="px-4 sm:px-6 py-4 border-t border-ds-border flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <span className="text-sm text-ds-muted">
                  {totalElements} ticket(s) · {currentPage * pageSize + 1}–{Math.min((currentPage + 1) * pageSize, totalElements)}
                </span>
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <span className="px-4 py-2 text-sm text-ds-primary">Page {currentPage + 1} sur {totalPages || 1}</span>
                <Button
                  variant="outline"
                  size="sm"
